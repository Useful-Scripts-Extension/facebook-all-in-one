...
import { fetchExtension, sendMessage } from './extension';
...
export function wrapGraphQlParams(params = {}) {
  ...
  for (const property in params) {
    ...
    const node = params[property as keyof typeof params];
...
export async function getAllFriends({ myUid, targetUid }: { myUid: string; targetUid?: string }) {
  const res = await fetchGraphQl({
...
export async function unfriend({ myUid, targetUid }: { myUid: string; targetUid: string }) {
  return fetchGraphQl({
