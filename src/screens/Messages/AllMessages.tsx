...
export default function AllMessages() {
  ...
  const onClickExport = (selectedData: Array<any>, type: string) => {
  ...
  const onClickFirstMessages = (record: TRecord) => () => {
  ...
  const columns: ColumnsType<TRecord> = [
        ...
        return (
          <Row align="middle">
            {record.isGroup ? (
              ...
              <Avatar shape="square" src={record.image || record.participants[0].avatar} />
            )}
    ...
    },
    {
      ...
      render: (text: string, record) => numberWithCommas(record.participants.length),
      width: 100,
      align: 'right',
    },
    {
      ...
      render: (text, record, index) => (
        <>
          ...
          <Tooltip title={t('Delete')}>
            <Button type="primary" danger icon={<i className="fa-solid fa-trash"></i>}></Button>
          </Tooltip>
        </>
      ),
      width: 150,
